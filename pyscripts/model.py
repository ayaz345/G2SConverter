...
from pyscripts.util import *
from pyscripts.BasicConvLSTMCell import *
...
class DEBLUR(object):
    ...
    def test(self, height, width, input_path, output_path):
        ...
        imgsName = [i for i in sorted(os.listdir(input_path)) if i.endswith(".png") or i.endswith(".PNG") or  i.endswith(".bmp") or  i.endswith(".BMP") or  i.endswith(".tga") or  i.endswith(".TGA") or  i.endswith(".jpg") or  i.endswith(".JPG") or  i.endswith(".JPEG") or  i.endswith(".jpeg")]
        H, W = height, width
        inp_chns = 3 if self.args.model == 'color' else 1
        self.batch_size = 1 if self.args.model == 'color' else 3
        inputs = tf.compat.v1.placeholder(
            shape=[self.batch_size, H, W, inp_chns], dtype=tf.float32)
        outputs = self.generator(inputs, reuse=tf.compat.v1.AUTO_REUSE)
